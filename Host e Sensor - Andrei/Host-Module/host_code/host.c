 ...
 * sendo esses definidos na library  'pacotes_inerciais.h'
 */
void uart_communication_handler();

/**
 * Trata os pacotes seriais recebidos. Os redireciona para a interface serial.
 */
void rf_communication_handler();

/**
 * Aguarda o recebimento de um pacote inercial vindo de um dado sensor.
 * @param sensor Sensor para aguardar sua resposta
 */
void wait_rf_response(uint8_t sensor);

/**
* Envia um dado comando cada sensor listado como ativo.
* Utiliza polling, ou seja aguardará a resposta do sensor.
